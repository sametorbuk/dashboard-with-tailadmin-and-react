import React from 'react';
import { Button, Modal, ModalHeader, ModalBody, ModalFooter } from 'reactstrap';
import 'bootstrap/dist/css/bootstrap.min.css';
import { booksData } from '../../books-data';
import { useEffect } from 'react';




function ProductAddComp(props) {
    const { addBookModal, toggle, setCurrentBookData, currentBookData } = props;



    useEffect(() => {
        console.log('Updated currentBookData:', currentBookData);
        localStorage.setItem("currentbookdata", JSON.stringify(currentBookData))
    }, [currentBookData]);


    const clickAddBookHandler = (event) => {
        event.preventDefault()
        const checkbox = event.target;
        const id = event.target.id
        if (checkbox.checked) {
            const selectedBook = booksData.find((book) => book.id == id);
            if (selectedBook) {
                setCurrentBookData([...currentBookData, selectedBook]);

            }
        } else {

            const updatedBooks = currentBookData.filter((book) => book.id !== id);
            setCurrentBookData(updatedBooks);
        }
    };

    return (
        <Modal isOpen={addBookModal} toggle={toggle} className='modal-lg'>
            <ModalHeader toggle={toggle}>Add Book</ModalHeader>
            <ModalBody>
                <div className=" rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
                    <div className="max-w-full overflow-x-auto">
                        <table className="w-full table-auto">
                            <thead>
                                <tr className="bg-gray-2 text-left dark:bg-meta-4">
                                    <th className="min-w-[220px] py-4 px-4 font-medium text-black dark:text-white xl:pl-11">
                                        Product Name
                                    </th>
                                    <th className="min-w-[150px] py-4 px-4 font-medium text-black dark:text-white">
                                        Publish Date
                                    </th>
                                    <th className="py-4 px-4 font-medium text-black dark:text-white">
                                        Select
                                    </th>
                                </tr>
                            </thead>
                            <tbody>
                                {booksData.map((book) => (
                                    <tr key={book.id}>
                                        <td className="border-b border-[#eee] py-5 px-4 pl-9 dark:border-strokedark xl:pl-11">
                                            <h5 className="font-medium text-black dark:text-white">
                                                {book.title}
                                            </h5>
                                            <p className="text-sm">${book.pageCount}</p>
                                        </td>
                                        <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                            <p className="text-black dark:text-white">
                                                {book.publishDate}
                                            </p>
                                        </td>
                                        <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                            <input
                                                type="checkbox"
                                                id={book.id}
                                                onChange={clickAddBookHandler}
                                            />
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                    </div>
                </div>
            </ModalBody>
            <ModalFooter>
                <Button color="primary" onClick={toggle}>
                    Add Selected
                </Button>{' '}
                <Button color="secondary" onClick={toggle}>
                    Cancel
                </Button>
            </ModalFooter>
        </Modal>
    );
}

export default ProductAddComp;
